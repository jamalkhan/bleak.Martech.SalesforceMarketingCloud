...
namespace bleak.Martech.SalesforceMarketingCloud.ConsoleApp.ConsoleApps
{
    public class DownloadOpensApp : IConsoleApp
    {
        ...
        public DownloadOpensApp(IRestClientAsync restClientAsync, IAuthRepository authRepository, string folder, int daysBack = 180)
        {
        ...
        private async Task ProcessDateAsync(DateTime startTime)
        {
            ...
            (
                ...
                logger: (ILogger<OpenEventSoapApi>)LogManager.GetLogger(typeof(OpenEventSoapApi).FullName ?? "OpenEventSoapApi"),
                startDate: startTime,
                endDate: endTime
            );
